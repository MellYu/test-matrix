...
const Matrix = ({ columnsNumber, matrix, rowsNumber }) => {
  useEffect(() => {
    const filedMatrix = Array(columnsNumber * rowsNumber)
      .fill(0)
      .map(randomNumber);
  ...
  const rowTotal = Array(rowsNumber)
    .fill(0)
    .map((item, rowIndex) =>
      matrix.reduce((acc, mat, i) => {
        if (
          i >= rowIndex * columnsNumber &&
          i < (rowIndex + 1) * columnsNumber
        ) {
  ...
  let columnAverage = [];
  if (rowTotal.length) {
    columnAverage = Array(columnsNumber)
      .fill(0)
      .map((item, columnIndex) => {
        let acc = 0;
        for (
          let i = columnIndex;
          i <= columnsNumber * rowsNumber - 1;
          i += columnsNumber
        ) {
  ...
  return (
    <div className={styles.matrix} style={gridShape}>
      ...
      {columnAverage.map((item) => (
        <ColumnAverage key={uuidv4()} amount={item} />
      ))}
    </div>
  );
};
